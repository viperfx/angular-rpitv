 ...
 */
tooglesApp.controller('ViewCtrl', ['$scope', '$routeParams', '$location', 'youtube', 'socket', function($scope, $routeParams, $location, youtube, socket) {
  ...
  $scope.streaming_text = 'Send to RaspberryPi'
  $scope.streaming = false;
  window.viewCallback = function(data) {
    if ($scope.section === 'view') {
      $scope.video = data.entry;
      $scope.video.video_id = $routeParams.id;
      $scope.video.embedurl = "http://www.youtube.com/embed/" + $scope.video.video_id + "?autoplay=0&theme=dark&color=white&iv_load_policy=3";
    } else {
      var start = $routeParams.start || 0;
      $scope.video = data.feed.entry[start];
      $scope.video.video_id = $scope.video.media$group.yt$videoid.$t;
      $scope.video.embedurl = "http://www.youtube.com/embed/videoseries?list=" + $routeParams.id + "&autoplay=0&theme=dark&color=white&iv_load_policy=3&index=" + start;
